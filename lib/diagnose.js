...
function diagnoseCrossCutting(pkg, docker, matrix) {
  ...
  const track = matrix.tracks[detectedTrack];
  const expectedNodes = expectedNodeTags(track);
  if (expectedNodes.length === 0) return out;

  if (expectedNodes.includes(nodeContainer.tag)) {
    out.push({
      id: 'node-track-match',
      severity: 'ok',
    ...
    out.push({
      ...
      severity: 'error',
      title: `midnight-node:${nodeContainer.tag} doesn't match SDK track (expects ${expectedNodes.join(' or ')})`,
      detail: `Detected SDK track: ${track.label}. Running node: ${nodeContainer.tag}. Mixing causes silent sync failures with no error.`,
      fix: `Either update the node container to one of ${expectedNodes.join(', ')}, or align SDK to a track that matches your node version.`,
    });
  }
...
function detectTrack(facadeVersion, matrix) {
  ...
  // Aligned with docs.midnight.network/relnotes/support-matrix as of 2026-04-27
  if (major === 3) return 'current';
  if (major === 4) return 'preview';
  if (major === 2) return 'legacy-v7-facade-2';
  if (major === 1) return 'archaeological-facade-1';
  return null;
}
...
function expectedNodeTags(track) {
  if (!track.node) return [];
  if (typeof track.node === 'string') return [track.node];
  // object form: { preview, preprod, mainnet }
  return Object.values(track.node);
}
